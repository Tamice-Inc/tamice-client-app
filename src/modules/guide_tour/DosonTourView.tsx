...
export const DosonTourView = () => {
  ...
  const { tickets } = useGetTickets({
    category: Number(menu[3]?.dropdownElements?.[2].category_id),
    subCategoryId: Number(menu[3]?.dropdownElements?.[2].id),
    menu,
    menuLoading
  });
  ...
  return (
    <div className="flex justify-center w-full gap-x-3">
      ...
      {tickets?.map((item:any) => (
        ...
        {/* {isShowing ? (
          <div className="flex justify-center text-base text-blue hover:cursor-pointer hover:underline font-poppins" onClick={handleClick}>
            전체보기
          </div>
        ) : (
          <></>
