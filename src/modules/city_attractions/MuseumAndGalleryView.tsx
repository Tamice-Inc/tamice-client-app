...
export const MuseumAndGallery = () => {
  ...
  // Fetch tickets from useGetTickets Custom Hook with API call
  const { tickets } = useGetTickets({
    category: Number(menu[2]?.dropdownElements?.[1].category_id),
    subCategoryId: Number(menu[2]?.dropdownElements?.[1].id),
    menu,
    menuLoading
  });

  // Create a navigate instance from useNavigate with API call
