...
const getDateTime = (date: Date) => {
  ...
  console.log("dateTime is ", formatDateTime(datetime));
  return datetime;
};

const getTicketDetail = async (id: number, setTicketDetail: any) => {
  return await axios
    .get(`${API}/tickets/${id}`)
    .then((data) => setTicketDetail(data.data));
};

export const MultiTourCalendar = (props: {
  order: any;
  filterCounter: any;
  handleClick: (date: string, id: string) => any;
  handleClickDisable: (date: string, id: string) => any;
  setIsOpen: any;
  selectedTour?: string;
}) => {
  ...
  useEffect(() => {
    // getTicketDetail(props.order, setTicketDetail);

    const getTicketDetail = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API}/tickets/${props.order}`);
  ...
  useEffect(() => {
    if (allow) {
      if (!!props.selectedTour) {
        ...
        const date = props.selectedTour?.[parseInt(props.order)];
        newDate.setFullYear(
          Number(date?.substring(0, 4)),
          Number(date?.substring(5, 7)) - 1,
          Number(date?.substring(8, 10))
  ...
  if (loading) {
    return (
      <div className="fixed top-0 left-0 z-50 w-full h-screen bg-black bg-opacity-25">
    ...
    <div className="container">
      <div className="calendar-container relative">
              ...
              return matchingTimes.length > 0 ? (
                <div className="mt-1">
                  {matchingTimes
                    .filter((item) => {
                      // Get current NY time
                      let estTime: any = new Date().toLocaleString("en-US", {
                      ...
                      const selectedTime: any = new Date(
                        `${formattedDate} ${item?.time?.substring(
                          0,
                          item?.time?.length - 2
                        )} ${item?.time?.substring(
                          item?.time?.length - 2,
                    ...
                    .map((matchingTime, index) => (
                      <div
                        ...
                        style={{
                          color:
                            matchingTime.check === 0 ? "#0572f7" : "#32a852",
                          paddingTop: "5px",
                          paddingBottom: "2px",
        ...
        <div className="flex justify-center w-52 absolute bottom-[20px] m-auto left-0 right-0">
          ...
          <button
            className="px-2 py-1 mr-2 text-xs text-white rounded bg-blue sm:py-2 sm:px-4 bg-[#5D5D5F] sm:text-[15px]"
            onClick={() => {
              props.setIsOpen(false);
              if (matchingTime) {
                changeColorWithDisable(matchingTime);
